...
class MyVSRModel(BaseModel):
    ...
    def train(self, data):
        ...
        lr_data1, gt_data1 = data['lr1'], data['gt1']
        ...
        # ------------ forward G ------------ #
        net_G_output_dict = self.net_G.forward_sequence(lr_data,lr_data1)
        ...
        if self.warp_crit is not None:
            ...
            lr_curr = net_G_output_dict['lr_curr'] #########da cambiare qua??????????'
    ...
    def infer(self, lr_data,lr_data1):
        ...
        lr_data1 = data_utils.canonicalize(lr_data1)  # to torch.FloatTensor
        lr_data1 = lr_data1.permute(0, 3, 1, 2)  # tchw

        # temporal padding
        lr_data, n_pad_front = self.pad_sequence(lr_data)
        lr_data1, n_pad_front = self.pad_sequence(lr_data1)

        # infer #A CHE SERVE TEMPORAL PADDING
        hr_seq = self.net_G.infer_sequence(lr_data,lr_data1,self.device)
