...
class FRNet(BaseSequenceGenerator):
    ...
    def generate_dummy_input(self, lr_size):
        ...
        return data_dict
    def forward(self, lr_curr, lr_prev, hr_prev,lr_curr2, lr_prev2, hr_prev2):
        """
            Parameters:
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        # pad if size is not a multiple of 8
        pad_h2 = lr_curr2.size(2) - lr_curr2.size(2) // 8 * 8
        pad_w2 = lr_curr2.size(3) - lr_curr2.size(3) // 8 * 8
        lr_flow_pad2 = F.pad(lr_flow2, (0, pad_w2, 0, pad_h2), 'reflect')

        # upsample lr flow
        hr_flow2 = self.scale * self.upsample_func(lr_flow_pad2)

        # warp hr_prev
        hr_prev_warp2 = backward_warp(hr_prev2, hr_flow2)

        # compute hr_curr
        hr_curr2 = self.srnet(lr_curr2, space_to_depth(hr_prev_warp2, self.scale))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
#first solution
        #up1 = torch.cat([lr_curr, space_to_depth(hr_prev_warp, self.scale)])
        #down1 = torch.cat([lr_curr2, space_to_depth(hr_prev_warp2, self.scale)])
        #hr_currTot1 = self.srnet(up1, down1)
#second solution
        #lr_currTot=lr_curr+lr_curr2
        #hr_prev_warpTot=hr_prev_warp+hr_prev_warp2
        #hr_currTot2 = self.srnet(lr_currTot, space_to_depth(hr_prev_warpTot, self.scale))
#third solution
        lr_currConc=torch.cat([lr_curr, lr_curr2])
        hr_prev_warpConc = torch.cat(hr_prev_warp,hr_prev_warp2)
        hr_currTot3 = self.srnet(lr_currConc, space_to_depth(hr_prev_warpConc, self.scale))
        return hr_curr2,hr_curr,hr_currTot3

    def forward_sequence(self, lr_data, lr_data1):
        """
            Parameters:
        ...
        lr_prev1 = lr_data1[:, :-1, ...].reshape(n * (t - 1), c, lr_h, lr_w)
        lr_curr1 = lr_data1[:, 1:, ...].reshape(n * (t - 1), c, lr_h, lr_w)
        lr_flow1 = self.fnet(lr_curr1, lr_prev1)  # n*(t-1),2,h,w

        # upsample lr flows
        hr_flow = self.scale * self.upsample_func(lr_flow)
        hr_flow = hr_flow.view(n, (t - 1), 2, hr_h, hr_w)

        hr_flow1 = self.scale * self.upsample_func(lr_flow1)
        hr_flow1 = hr_flow1.view(n, (t - 1), 2, hr_h, hr_w)

        # compute the first hr data
        hr_data = []
        lr_currConc = torch.cat([lr_data[:, 0, ...], lr_data1[:, 0, ...]])
        hr_currTot3 = self.srnet(lr_currConc, torch.zeros(n, (self.scale ** 2) * c, lr_h, lr_w, dtype=torch.float32,
                        device=lr_data.device))
        hr_data.append(hr_currTot3)


        # compute the remaining hr data
        for i in range(1, t):  #con third solution
            # warp hr_prev
            hr_prev_warp = backward_warp(hr_prev, hr_flow[:, i - 1, ...])
            hr_prev_warp1 = backward_warp(hr_prev1, hr_flow1[:, i - 1, ...])
            # compute hr_curr
            hr_curr = self.srnet(
                lr_data[:, i, ...],
            ...
            hr_curr1 = self.srnet(
                lr_data1[:, i, ...],
                space_to_depth(hr_prev_warp1, self.scale))
            lr_currConc = torch.cat([lr_data[:, i, ...], lr_data1[:, i, ...]])
            hr_prev_warpConc = torch.cat(hr_prev_warp, hr_prev_warp1)
            hr_currTot3 = self.srnet(lr_currConc, space_to_depth(hr_prev_warpConc, self.scale))
        ...
        # hr_data1 = torch.stack(hr_data1, dim=1)
        # construct output dict
        ret_dict = {  # qua pero non va bene così #######prob ne devo fare uno solo con tutti i dati
            ...
            'hr_data1': hr_data1,  # n,t,c,hr_h,hr_w
            'hr_flow1': hr_flow1,  # n,t,2,hr_h,hr_w
            'lr_prev1': lr_prev1,  # n(t-1),c,lr_h,lr_w
            'lr_curr1': lr_curr1,  # n(t-1),c,lr_h,lr_w
            'lr_flow1': lr_flow1,
        }
    ...
    def infer_sequence(self, lr_data1,lr_data2, device):
        """
            Parameters:
        ...
        # setup params
        tot_frm1, c1, h1, w1 = lr_data1.size()
        tot_frm2, c2, h2, w2 = lr_data2.size()
        s = self.scale

        # forward
        hr_seq = []
        lr_prev1 = torch.zeros(1, c1, h1, w1, dtype=torch.float32).to(device)
        hr_prev1 = torch.zeros(
            1, c1, s * h1, s * w1, dtype=torch.float32).to(device)
        lr_prev2 = torch.zeros(1, c2, h2, w2, dtype=torch.float32).to(device)
        hr_prev2 = torch.zeros(
            1, c2, s * h2, s * w2, dtype=torch.float32).to(device)

        for i in range(tot_frm1):
            with torch.no_grad():
                self.eval()

                lr_curr1 = lr_data1[i: i + 1, ...].to(device)
                lr_curr2 = lr_data2[i: i + 1, ...].to(device)
                hr_curr2,hr_curr,hr_currTot = self.forward(lr_curr1, lr_prev1, hr_prev1,lr_curr2, lr_prev2, hr_prev2)
                lr_prev1, hr_prev1,lr_prev2,hr_prev2 = lr_curr1, hr_curr,lr_curr2,hr_curr2

                hr_frm = hr_currTot.squeeze(0).cpu().numpy()  # chw|rgb|uint8
